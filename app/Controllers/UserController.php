 ...
 * @property-read AliyunSmsService $aliyunSmsService
 */
class UserController extends Controller
{
    ...
    {
        $user_name = input('user_name', ['account', 'maxLength' => 20]);
        $password = input('password', ['required', 'maxLength' => 16]);
        $real_name = input('real_name', ['default' => '', 'maxLength' => 20]);
        $code = input('code');
    ...
    public function smsAction()
    {
        ...
        if ($this->redis->get($prefix . 'nums:' . $mobile) >= 3 && $this->redis->get($prefix . 'firsttime:' . $mobile) !== false) {
            return '你的验证码获取已达上限，请30分钟后再来获取';
        }

        $code = $this->random->getInt(100000, 999999);
